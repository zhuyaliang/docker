...
SELECT DISTINCT CAST(SYS_GET_USERBYID(c.relowner) AS VARCHAR(30 BYTE)) 
		...
		, CAST((CASE WHEN  c.RELNAMESPACE = sys_my_temp_schema() THEN 'Y' ELSE 'N' END) AS VARCHAR(1 BYTE))
		, CAST('N' AS VARCHAR(1 BYTE))
...
-- bug#23058:	AND roldatabase = (SELECT OID FROM SYS_DATABASE WHERE DATNAME = CURRENT_DATABASE());
...
SELECT (sys_get_userbyid(c.relowner))::CHARACTER VARYING(30 BYTE) AS "OWNER" ,
(cs1.CONNAME)::CHARACTER VARYING(30 BYTE) AS "CONSTRAINT_NAME",
(
...
SYS_GET_USERBYID(c_ref.RELOWNER)::CHARACTER VARYING(30 BYTE) AS "R_OWNER",
(cs2.CONNAME)::CHARACTER VARYING(30 BYTE) AS "R_CONSTRAINT_NAME",
  (
...
(
    ...
    (
      ...
      WHEN (cs1.CONTYPE = 'c'::"CHAR") THEN 'ENABLED'::TEXT
      ELSE NULL::TEXT END
...
(
  CASE WHEN (STATUS='ENABLED'::TEXT) THEN 'VALIDATED'::TEXT
       WHEN (STATUS='DISABLED'::TEXT) THEN 'NOVALIDATED'::TEXT
      ELSE NULL::TEXT END
...
(	
	...
	ELSE
		NULL
	END
 )::CHARACTER VARYING(30 BYTE) AS "INDEX_OWNER",
 (	
	CASE WHEN (cs1.CONTYPE = 'p' OR cs1.CONTYPE = 'u') THEN
		(
	...
	ELSE
		NULL
	END
 )::CHARACTER VARYING(30 BYTE) AS "INDEX_NAME",
...
from( sys_constraint cs1 left outer join sys_constraint cs2 on 
		(
			/* bug#22563: bad r_constraint_name value. */
...
select 
		...
		cast(c.conname as character varying(30 byte)) as CONSTRAINT_NAME,
		cast((case c.contype when 'c' then 'C' when 'p' then 'P' when 'f' then 'R' when 'u' then 'U' end) as character varying(1 byte)) as CONSTRAINT_TYPE,
		cast(r.relname as character varying(30 byte)) as TABLE_NAME,
		(select cast(af.usename as character varying(30 byte)) from sys_catalog.sys_constraint cf, sys_catalog.sys_class rf, sys_catalog.sys_user af
		where cf.confrelid = rf.oid and rf.relowner = af.usesysid and cf.oid = c.oid) as R_OWNER,
		(select cast(r.relname||'_PKEY' as character varying(30 byte)) from sys_catalog.sys_constraint c1, sys_catalog.sys_class r1 
		where c1.confrelid = r1.oid and c1.oid = c.oid ) as R_CONSTRAINT_NAME,
		cast((case c.confdeltype when 'a' then 'NO ACTION' when 'r' then 'STRICT' when 'c' then 'CASCADE' when 'n' then 'SET NULL' when 'd' then 'DEFAULT' end ) as character varying(9 byte)) as DELETE_RULE,
		cast((case c.constatus when true then 'ENABLED' else 'DISABLED' end) as character varying(8 byte)) as STATUS,
		cast((case c.condeferrable when true then 'DEFERRABLE'  else 'NOT DEFERRABLE' end) as character varying(14 byte)) as DEFERABLE,
		cast((case c.condeferred when true then 'DEFERRED' else 'IMMEDIATE' end) as character varying(9 byte))as DEFERRED
		from sys_catalog.sys_constraint c, sys_catalog.sys_class r, sys_catalog.sys_user a
		where c.conrelid = r.oid and r.relowner = a.usesysid;
...
create or replace view dba_data_files as
select 
	cast(df.filename as character varying(513 char)) as FILE_NAME,
	cast(cast(df.fileid as bigint ) as number(38,0)) as FILE_ID,
	cast(df.spcname as character varying(30 byte)) as TABLESPACE_NAME ,
	cast(cast(getBlockSize() as number(38,0)) * df.currentblocks as number(38,0)) as BYTES,
	cast(df.currentblocks as number(38,0)) as BLOCKS,
	cast('AVAILABLE' as character varying(9 byte)) as STATUS,
	cast(cast(df.fileid as bigint) as number(38,0)) as RELATIVE_FNO,
	cast ((case when df.initsize < df.maxsize then 't'
             else 'f' end) as character varying(3 byte))
      as AUTOEXTENSIABLE,
	cast((cast((1024*1024) as number(38,0))*df.maxsize) as number(38,0)) as MAXBYTES,
	cast(128*df.maxsize as number) as MAXBLOCKS,
	cast(cast(df.currentblocks as number(38,0))*df.growth/100 as number(38,0)) as INCREMENT_BY,
	cast(cast(getBlockSize() as number(38,0))* df.userblocks as number(38,0)) as USER_BYTES,
	cast(df.userblocks as number(38,0)) as USER_BLOCKS,
	cast((case when df.status then 'ONLINE'
	           else 'OFFLINE' end) as character varying(7 byte)) as ONLINE_STATUS
	from sys_catalog.sys_datafiles df;
...
FROM  sys_catalog.SYS_user au,sys_catalog.SYS_CLASS C left join sys_catalog.SYS_DESCRIPTION d 
      on (c.oid = d.objoid and d.classoid = 1259 and d.objsubid=0)
where c.relowner=au.usesysid
      and c.relkind in ('r','v');
...
select
	...
	cast(CAST(1024 AS NUMBER(38,0))*1024*16000*1024 as number(38,0)) as MAX_SIZE,
	cast((select sum(cast(df.currentblocks as number(38,0))) from sys_datafiles df) as number(38,0)) as TOTAL_SIZE,
	cast('ONLINE' as character varying(10 byte)) as STATUS$
	from sys_catalog.sys_database db;
...
select 
	...
	cast('ALLOWED' as character varying(10 byte)) as LOGINS,
