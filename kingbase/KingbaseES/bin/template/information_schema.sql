...
CREATE INTERNAL FUNCTION _sys_keyissubset(smallint[], smallint[]) RETURNS boolean
    LANGUAGE sql
    IMMUTABLE
    RETURNS NULL ON NULL INPUT
    AS 'select $1[1] is null or ($1[1] = any ($2) and coalesce(information_schema._sys_keyissubset($1[2:sys_catalog.array_upper($1,1)], $2), true))';

CREATE INTERNAL FUNCTION _sys_keysequal(smallint[], smallint[]) RETURNS boolean
    LANGUAGE sql
    IMMUTABLE
    RETURNS NULL ON NULL INPUT
    AS 'select information_schema._sys_keyissubset($1, $2) and information_schema._sys_keyissubset($2, $1)';

/* Get the OID of the unique index that an FK constraint depends on */
CREATE INTERNAL FUNCTION _sys_underlying_index(oid) RETURNS oid
    LANGUAGE sql STRICT STABLE
    AS $$
SELECT refobjid FROM sys_catalog.sys_depend
  WHERE classid = 'SYS_CATALOG.SYS_CONSTRAINT'::sys_catalog.regclass AND
        objid = $1 AND
        refclassid = 'SYS_CATALOG.SYS_CLASS'::sys_catalog.regclass AND
...
CREATE INTERNAL FUNCTION _sys_index_position(oid, smallint) RETURNS int
    ...
    AS $$
SELECT (ss.a).n FROM
  (SELECT information_schema._sys_expandarray(indkey,null,null) AS a
...
CREATE DOMAIN character_data AS character varying(8000);
...
CREATE DOMAIN sql_identifier AS character varying(8000);
...
CREATE VIEW check_constraints AS
    ...
    WHERE n.oid = r.relnamespace
      ...
      AND a.attnotnull
      AND r.relkind = 'r' AND r.relparttyp != 'p'
...
CREATE VIEW column_domain_usage AS
    ...
    WHERE t.typnamespace = nt.oid
          ...
          AND a.attnum > 0
          AND NOT a.attisdropped
          AND c.relowner = (SELECT oid FROM sys_authid a WHERE current_user=a.rolname);
...
CREATE VIEW column_privileges AS
    SELECT CAST(
		   		CASE WHEN u_grantor.rolname = grantee.rolname 
					 THEN '_SYSTEM'
		 	  		 ELSE u_grantor.rolname 
				END AS sql_identifier) AS grantor,
           ...
           CAST(
             CASE WHEN
                  ...
                  OR aclcontains(a.attacl,
                                 makeaclitem(grantee.oid, u_grantor.oid, pr.type, true))
                  OR u_grantor.rolname = grantee.rolname
    ...
    WHERE a.attrelid = c.oid
          ...
          AND (
				(c.relacl is null 
					AND u_grantor.rolname = grantee.rolname
					AND sys_get_userbyid(c.relowner) = current_user
				)
				OR aclcontains(c.relacl,
          ...
          AND (u_grantor.rolname = current_user
               OR grantee.rolname = current_user
...
CREATE VIEW columns AS
    SELECT CAST(current_database() AS sql_identifier) AS table_catalog,
           ...
           CAST(
             ...
             THEN
                (SELECT 
                  	CASE WHEN adsrc = '"CURRENT_USER"()'
                  	THEN 'USER'
                  	ELSE split_part(adsrc, '::', 1)
                    END
                  FROM  sys_attrdef 
                  WHERE adrelid = sys_viewcol_get_parent_relid(c.oid, a.attnum)
                      AND adnum = sys_viewcol_get_parent_attnum(c.oid, a.attnum))
             ELSE
                  CASE WHEN ad.adsrc = '"CURRENT_USER"()'
                  THEN 'USER'
                  ELSE split_part(ad.adsrc, '::', 1)
                  END
             END
             AS character_data
                ) AS column_default,
           ...
           CAST(
             ...
             THEN
                (SELECT 
                  CASE WHEN view_nullable IS NOT NULL THEN view_nullable
                  ELSE 'YES'
                  END
                FROM (SELECT 'temp' AS temp) left join
	                 (SELECT 
	                    CASE WHEN local_att.attnotnull OR 
	                        (local_type.typtype = 'd' AND local_type.typnotnull) 
	                    THEN 'NO'
	                    ELSE 'YES'
	                    END AS view_nullable
	                  FROM sys_attribute local_att, sys_type local_type
	                  WHERE local_att.atttypid = local_type.oid
	                      AND local_att.attrelid = sys_viewcol_get_parent_relid(c.oid, a.attnum)
	                      AND local_att.attnum = sys_viewcol_get_parent_attnum(c.oid, a.attnum))
	                  on true)
             ELSE
           ...
           CAST(
		     CASE WHEN t.typtype = 'd' THEN
		       CASE WHEN t.typbasetype IN (1186) AND t.typtypmod <> -1
		       		THEN sys_interval_type(t.typtypmod)
		       END
		     ELSE
		       CASE WHEN a.atttypid IN (1186) AND a.atttypmod <> -1
		       		THEN sys_interval_type(a.atttypmod)
		       END
		     END
			 AS character_data)
		     AS interval_type,
		   ...
		   CAST(
		     CASE WHEN t.typtype = 'd' THEN
		       CASE WHEN t.typbasetype IN (1186)
					THEN (CASE WHEN t.typtypmod <> -1 THEN sys_interval_prec1(t.typtypmod) ELSE null END)
		       END
		     ELSE
		       CASE WHEN a.atttypid IN (1186)
					THEN (CASE WHEN a.atttypmod <> -1 THEN sys_interval_prec1(a.atttypmod) ELSE null END)
		       END
		     END
			 AS character_data)
           ...
           CAST(CASE WHEN c.relkind = 'r' THEN 'YES'
                     WHEN c.relkind = 'v'
                     AND sys_view_is_updatable(c.oid) IN (0, 2)
                     AND sys_viewcol_is_updatable(c.oid, a.attnum) = true
                     THEN 'YES'
                     ELSE 'NO' END AS character_data) AS is_updatable
    ...
    WHERE a.attrelid = c.oid
          ...
          AND a.attnum > 0 AND NOT a.attisdropped AND c.relkind in ('r', 'v')
          AND c.relparttyp != 'p'
          AND (c.relowner = (select oid from sys_authid a where a.rolname = current_user) 
				/* bug#11482: attributes has column privileges */
...
CREATE VIEW constraint_column_usage AS
    ...
    WHERE x.tblowner = (SELECT oid FROM sys_authid a WHERE current_user = a.rolname);
...
CREATE VIEW constraint_table_usage AS
    ...
    WHERE c.connamespace = nc.oid AND r.relnamespace = nr.oid
          AND ( (c.contype = 'f' AND c.confrelid = r.oid)
             OR (c.contype IN ('p', 'u', 'c') AND c.conrelid = r.oid) )
          AND r.relkind = 'r' AND r.relparttyp != 'p'
          AND r.relowner = (SELECT oid FROM sys_authid a WHERE current_user = a.rolname);
...
CREATE VIEW key_column_usage AS
    SELECT CAST(current_database() AS sql_identifier) AS constraint_catalog,
           ...
           CAST(CASE WHEN contype = 'f' THEN
                  _sys_index_position(_sys_underlying_index(ss.coid),
                                     ss.confkey[(ss.x).n])
                     ELSE NULL
    ...
    FROM sys_attribute a,
         (SELECT r.oid AS roid, r.relname, nc.nspname AS nc_nspname,
                 ...
                 _sys_expandarray(c.conkey,null,null) AS x
...
CREATE VIEW referential_constraints AS
    ...
    FROM (sys_namespace ncon
          ...
          INNER JOIN sys_class c ON con.conrelid = c.oid)
         LEFT JOIN
         (sys_constraint pkc
          INNER JOIN sys_namespace npkc ON pkc.connamespace = npkc.oid)
         ON con.confrelid = pkc.conrelid
            AND _sys_keysequal(con.confkey, pkc.conkey)
...
CREATE VIEW schemata AS
    ...
    WHERE n.nspowner = u.oid AND u.rolname = current_user;
...
CREATE VIEW table_privileges AS
    ...
    WHERE c.relnamespace = nc.oid
          ...
          AND c.relparttyp != 'p'
          AND (
				((c.relacl is null) AND (u_grantor.rolname = grantee.rolname) AND (SYS_GET_USERBYID(c.relowner) = current_user))
				OR aclcontains(c.relacl,
                          makeaclitem(grantee.oid, u_grantor.oid, pr.type, false))
				)
          AND (u_grantor.rolname = current_user
...
CREATE VIEW tables AS
    SELECT CAST(current_database() AS sql_identifier) AS table_catalog,
           ...
           CAST(CASE WHEN c.relkind = 'r' THEN 'YES'
                     ...
                     AND sys_view_is_updatable(c.oid) IN (0, 2)
                     THEN 'YES'
                     ELSE 'NO' END AS character_data) AS is_insertable_into,
           CAST('NO' AS character_data) AS is_typed,
           CAST(
             CASE WHEN nc.oid = sys_my_temp_schema() THEN 'PRESERVE' -- FIXME
                  ELSE null END
    ...
    WHERE c.relnamespace = nc.oid
          ...
          AND (c.relowner = (select oid from sys_authid a where a.rolname = current_user )
               ...
               /* bug#12028: has column privileges */
...
-- Kingbase doesn't allow the specification of individual triggered
...
-- Of the things currently implemented in Kingbase, usage privileges
...
CREATE VIEW view_column_usage AS
    ...
    WHERE nv.oid = v.relnamespace
          ...
          AND v.relowner = (SELECT oid FROM sys_authid a WHERE current_user = a.rolname);
...
CREATE VIEW views AS
    SELECT CAST(current_database() AS sql_identifier) AS table_catalog,
           ...
           CAST(
             CASE WHEN EXISTS (SELECT 1 FROM sys_rewrite WHERE ev_class = c.oid AND ev_type = 3 AND is_instead)
                  THEN 'YES' 
                  WHEN c.relkind = 'v'
                   AND sys_view_is_updatable(c.oid) IN (0, 2) THEN 'YES'
                  ELSE 'NO' END
    ...
    FROM sys_namespace n, sys_type at, sys_namespace nbt, sys_type bt,
         (
           ...
           FROM (SELECT p.pronamespace, p.proname, p.oid,
                        _sys_expandarray(p.proargtypes::oid[],null,null) AS x
                 FROM sys_proc p WHERE p.propkgoid = 0) AS ss
           ...
           FROM sys_proc p WHERE p.propkgoid = 0
