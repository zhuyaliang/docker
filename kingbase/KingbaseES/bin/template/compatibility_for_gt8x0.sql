...
SELECT    CAST(u.usename AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST(relname AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
...
UNION
SELECT    CAST(u.usename AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST(l.relname AS VARCHAR2(128 BYTE))
        , CAST(c.relname AS VARCHAR2(128 BYTE)) 
        , CAST(CAST(c.oid AS VARCHAR(38 BYTE)) AS NUMERIC(38,0))
        , CAST(CAST(c.oid AS VARCHAR(38 BYTE)) AS NUMERIC(38,0))
        , CAST(
			(CASE c.relkind 
...
SELECT    CAST(current_user AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST('TRIGGER_PKG' AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
...
SELECT    CAST(current_user AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST('OPEN2000E_PKG' AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
        , CAST(0 AS NUMERIC(38,0))
        , CAST(NULL AS NUMERIC(38,0))
        , CAST('PACKAGE'  AS VARCHAR2(19 BYTE))
...
SELECT    CAST(current_user AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST('FORM_TM_ND_TAP_MEAS' AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
        , CAST(0 AS NUMERIC(38,0))
        , CAST(NULL AS NUMERIC(38,0))
        , CAST('PACKAGE'  AS VARCHAR2(19 BYTE))
...
SELECT    CAST(current_user AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST('NEW_STATISTICS_SAMPLE_PKG' AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
        , CAST(0 AS NUMERIC(38,0))
        , CAST(NULL AS NUMERIC(38,0))
        , CAST('PACKAGE'  AS VARCHAR2(19 BYTE))
...
SELECT    CAST(u.usename AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST(proname AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
...
SELECT    CAST(u.usename AS VARCHAR2(63 BYTE)) -- TODO: Oracle uses 32.
        , CAST(pkgname AS VARCHAR2(128 BYTE))
        , CAST(NULL AS VARCHAR2(128 BYTE))
...
CREATE OR REPLACE VIEW all_objects
	...
	WHERE owner = CAST(current_user AS VARCHAR2(63 BYTE))
...
CREATE VIEW ALL_COL_PRIVS AS
	...
	WHERE grantee='PUBLIC'
	      ...
	      OR grantee=cast(current_user as varchar(63 byte))
		  OR owner=cast(current_user as varchar(63 byte))
          OR grantee in (select cast(r.rolname as varchar(63 byte)) from sys_authid r,sys_authid u where u.roldefault=r.oid
                         UNION ALL
                         select cast(current_role as varchar(63 byte)));
...
select 
	s.owner::CHARACTER VARYING(63 BYTE) as OWNER,
	s.synname::CHARACTER VARYING(63 BYTE) as SYNONYM_NAME,
	if (s.dblink is null, sys_get_userbyid(c.relowner), null)::CHARACTER VARYING(63 BYTE) as TABLE_OWNER,
	s.objname::CHARACTER VARYING(63 BYTE) as TABLE_NAME,
...
from 
	...
	left outer join sys_catalog.sys_class c on (s.objname = c.relname and c.relnamespace = n.oid)
where
	s.owner = (cast(current_user as varchar2(63 byte)))
	or 
...
union
	...
	select s.synname::CHARACTER VARYING(63 BYTE), ('Synonym for ' || s.objname)::CHARACTER VARYING(4000 CHAR)
...
select * from sys_catalog.dba_source where owner = CAST("CURRENT_USER"() AS CHARACTER VARYING(63 BYTE));
...
cast(df.spcname as varchar2(63 BYTE)) as tablespace_name,	  
...
select 
    ...
    cast(d.DESCRIPTION as character varying(4000 char)) AS COMMENTS
	FROM SYS_CATALOG.SYS_user a, SYS_CATALOG.SYS_ATTRIBUTE at, SYS_CATALOG.SYS_CLASS C, SYS_CATALOG.SYS_DESCRIPTION d
	WHERE at.attrelid = c.oid and d.classoid = 1259 
      and at.attnum=d.objsubid 
      and c.relkind in ('r','v')
      and d.objoid=at.attrelid
...
SELECT 
	   cast(SA.USENAME as character varying(63 byte)) AS OWNER,
       cast(SC.RELNAME as character varying(63 byte)) AS TABLE_NAME,
       ...
       cast('SYSTEM' as character varying(63 byte)) AS DEF_TABLESPACE_NAME,
...
SELECT cast(SA.USENAME as character varying(63 byte)) AS TABLE_OWNER,
       cast(SC.RELNAME as character varying(63 byte)) AS TABLE_NAME,
       cast(CASE WHEN C.OID IN (SELECT PRELID FROM SYS_PARTCLASS)
            THEN 'YES'
            ELSE 'NO'
            END as character varying(3 byte)) AS COMPOSITE,
       cast(C.RELNAME as character varying(63 byte)) AS PARTITION_NAME,
       cast(CASE WHEN C.OID IN (SELECT PRELID FROM SYS_PARTCLASS)
            THEN (SELECT PRELNPARTS FROM SYS_PARTCLASS WHERE PRELID=C.OID)
            ELSE 0
       ...
       cast(STS.SPCNAME as character varying(63 byte)) AS TABLESPACE_NAME,
...
select 
	cast(dl.lnkowner as character varying(63 byte)) as OWNER,
	cast(dl.lnkname as character varying(128 byte))  as DB_LINK,
	cast(dl.lnkuser as character varying(63 byte)) as USERNAME,
...
CAST(a.usename as varchar2(63)) AS ORACLE_USERNAME,
CAST(null as varchar2(63)) AS OS_USER_NAME,
